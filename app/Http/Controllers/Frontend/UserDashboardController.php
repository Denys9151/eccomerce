...
class UserDashboardController extends Controller
{
    public function index()
    {
        ...
        $completeOrder = Order::where('user_id', Auth::id())->where('order_status', 'delivered')->count();
        $reviews = ProductReview::where('user_id', Auth::id())->count();
        $wishlist = Wishlist::where('user_id', Auth::id())->count();

        return view('frontend.dashboard.dashboard', compact('totalOrder', 'pendingOrder', 'completeOrder', 'reviews', 'wishlist'));
    }
}
