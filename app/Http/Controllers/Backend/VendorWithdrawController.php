...
class VendorWithdrawController extends Controller
{
    ...
    public function index(VendorWithdrawDataTable $dataTable)
    {
        ...
        return $dataTable->render('vendor.withdraw.index',compact('currentBalance', 'pendingAmount', 'totalWithdraw'));
    }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'account_info' => ['required', 'string', 'max:2048'],
        ]);
        ...
        if(WithdrawRequest::where(['vendor_id' => auth()->id(), 'status' => 'pending'])->exists()){
        ...
        toastr('Request added successfully!', 'success', 'Success');
    ...
    {
        $request = WithdrawRequest::where('vendor_id', auth()->id())->findOrFail($id);
